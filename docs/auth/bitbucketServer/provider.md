...
When setting up the incoming link, it is essential to configure it with the following redirect URL: `https://<your-backstage>/api/auth/bitbucketServer/handler/frame` (for local development you should use http://localhost:7007/api/auth/bitbucketServer/handler/frame).

Replace <your-backstage> with your Backstage instance's hostname. 
